...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Windows から router への SSH ログイン過程のパケットを router の Wireshark で解析し，iptables による転送（IP アドレスやポート番号の書き換え）が観測できるパケットを特定する．
        ...
        tcpのヘッダにおいて, フィールドの値(\texttt{Sequence Number(raw): 3645530960, Acknowledgment Number(raw): 1354975756 など})が完全一致しており,
        転送されているものであるといえる.
    ...
    \item server から組織外ネットワークの端末へ通信を発生させ，router の Wireshark で解析し，iptables による変換（送信元 IP の書き換え等）を示すパケットを特定する．
        ...
        \quad
        図\ref{fig:masquarade}は, serverからwindowsに対してpingコマンドを実行した結果である.
        \begin{figure}[htbp]
            \centering
            \includegraphics[width=0.8\textwidth]{imgs/masquerade.png} 
            \caption{serverからwindowsに対するpingコマンドの実行結果}
            \label{fig:masquarade}
        \end{figure}

        \quad
        ターミナルでの実行結果からwindowsからの応答が確認でき, serverとwindowsとの間で通信が可能であることが確認できる.\\
        \quad
        wiresharkでのキャプチャ結果は,
        左がeth0でのキャプチャ結果, 右がeth1でのキャプチャ結果である.
        両者で類似したパケットが確認できる.\\
        \quad
        図\ref{fig:masquerade_comp}にserverからwindowsに対するpingコマンドの実行時に
        eth0とeth1で観測されたパケットの例を示す.
        \begin{figure}[htbp]
            \centering
            \includegraphics[width=0.8\textwidth]{imgs/masquerade_compare_packet.png} 
            \caption{serverからwindowsに対するpingコマンドの実行時にeth0とeth1で観測されるパケットの比較}
            \label{fig:masquerade_comp}
        \end{figure}      
        
        \quad
        ここでは, pingコマンドによって発生した
        ICMPエコー要求のパケットの例を上げた.
        両パケットでIdentifierとSequesns\, Numberが一致していることから
        同一のパケットであることがわかる.
        eth0において
        server(192.168.100.2)からwindows(10.6.158.153)に送信されて,
        routerに到達したパケットが
        eth1から
        router(10.6.158.90)からwindows(10.6.158.153)へのパケットして,
        送信元のIPアドレスがserverからrouterに書き換えられて転送されていることが確認できる.
